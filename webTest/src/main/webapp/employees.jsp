...
<body>
	<form action="employees" method="get">
		<input type="hidden" name="operation" value="search"> <select
			name="searchType">
	...
	<table>
		...
		<c:forEach items="${emps}" var="e">
			<%-- second type of writing style , first is below it --%>
			<tr>
				<td><c:out value="${e.id}"></c:out></td>
				<%-- c out is used for writing in a good way . can be used and not --%>
				<td>${e.name}</td>
				<td>${e.age}</td>
				<td>${e.gender}</td>
				<td>${e.salary}</td>
		...
		<tr>
			<td colspan="7"><a href="employees?operation=new">New
					Employee</a>
		</tr>
