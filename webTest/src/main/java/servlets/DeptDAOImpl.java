...
public class DeptDAOImpl implements DeptDAO {
	...
	public Dept next(int id) {
		try (Connection conn = getConnection()) {
			System.out.println(
					" Executing SQL: SELECT * FROM dept WHERE deptid > " + id + " ORDER BY deptid ASC LIMIT 1");

			PreparedStatement ps = conn
					.prepareStatement("SELECT * FROM dept WHERE deptid > ? ORDER BY deptid ASC LIMIT 1");
			ps.setInt(1, id);
			ResultSet rs = ps.executeQuery();

			if (rs.next()) {
				System.out.println("Next Department Found: " + rs.getString("deptname"));
				return new Dept(rs.getInt("deptid"), rs.getString("deptname"), rs.getString("deptlocation"));
			} else {
	...
	public Dept previous(int id) {
		try (Connection conn = getConnection()) {
			System.out.println(
					" Executing SQL: SELECT * FROM dept WHERE deptid < " + id + " ORDER BY deptid DESC LIMIT 1");

			PreparedStatement ps = conn
					.prepareStatement("SELECT * FROM dept WHERE deptid < ? ORDER BY deptid DESC LIMIT 1");
			ps.setInt(1, id);
			ResultSet rs = ps.executeQuery();

			if (rs.next()) {
				System.out.println(" Previous Department Found: " + rs.getString("deptname") + " (ID: "
						+ rs.getInt("deptid") + ")");
				return new Dept(rs.getInt("deptid"), rs.getString("deptname"), rs.getString("deptlocation"));
			} else {
	...
	public void save(Dept dept) {
		try (Connection conn = getConnection()) {
			...
			PreparedStatement ps = conn
					.prepareStatement("INSERT INTO dept (deptid, deptname, deptlocation) VALUES (?, ?, ?)");
	...
	public void update(Dept dept) {
		try (Connection conn = getConnection()) {
			...
			PreparedStatement ps = conn
					.prepareStatement("UPDATE dept SET deptname = ?, deptlocation = ? WHERE deptid = ?");
	...
	public Set<Dept> getAll() {
		...
		try (Connection conn = getConnection()) {
			...
			while (rs.next()) {
				depts.add(new Dept(rs.getInt("deptid"), rs.getString("deptname"), rs.getString("deptlocation")));
			}
		...
		try (Connection conn = getConnection()) {
			PreparedStatement ps = conn.prepareStatement(
					"SELECT e.id, e.name, e.age, e.gender, e.salary, e.experience, e.level, d.deptname, d.deptlocation "
							+ "FROM employee e " + "JOIN dept d ON e.deptid = d.deptid " + "WHERE e.deptid = ?");
			ps.setInt(1, deptId);
			ResultSet rs = ps.executeQuery();
			while (rs.next()) {
				Employee emp = new Employee(rs.getInt("id"), rs.getString("name"), rs.getInt("age"),
						Employee.Gender.valueOf(rs.getString("gender")), rs.getFloat("salary"), rs.getInt("experience"),
						rs.getInt("level"), deptId);
